...
1. 概率相关公式及贝叶斯定理
    ...
    - 全概率：若事件 B1, B2, ..., Bn 构成一个**完备事件组**，即他们两两不相容，且和为全集，则对于任意事件 A 有：<br> $$p(A)=\sum_{i=1}^n p(A|B_i)*p(B_i)$$
...
1. SVM 的核心思想？
    ...
    SVM 是定义在特征空间中的线性分类器，目标是使得分隔超平面 $(wx_i+b)$ 两边的数据点离超平面的**间隔最大**。当数据线性可分时，分隔超平面两边的数据点满足：若 $y_i=1$, $(wx_i+b) \geq 1$ , 若 $y_i=-1$, $(wx_i+b) \leq -1$ (综合起来 $y_i(wx_i+b)-1 \geq 0$ ), 可以通过**硬间隔最大化**来学习一个线性的超平面；当数据近似线性可分时，则可以通过加上一个松弛变量，通过**软间隔最大化**来学习超平面；当线性不可分时，也可以使用核函数，将输入空间映射到高维的特征空间。

3. 硬间隔最大化

    首先定义超平面 $(w,b)$ 关于样本点 $(x_i,y_i)$ 的**函数间隔**为 $\hat{\gamma}_i= y_i(wx_i+b)$ （其中 $y_i=-1, 1$ )

但是，如果成比例地改变 $w$ 和 $b$ 的值，函数间隔会发生改变，但其超平面并没有变。为了解决这个问题，引入**几何间隔**为超平面与样本点的直线距离 $\gamma_i=y_i(\frac{w}{||w||}x_i+\frac{b}{||w||})$  
硬间隔最大化的优化目标就是找到合适的超平面 $(w,b)$，使得点到超平面的间隔最大，即：
...
2. 逻辑回归的损失函数与梯度
   ...
   = -\sum_i \frac{\partial [y_i z_i - \ln(1 + e^{z_i})]}{\partial z_i} \cdot \frac{\partial z_i}{\partial w_j}
...
此外，XGBoost 在每个叶子节点上都增加了正则化项 $$\Omega(f_t) = \gamma T + \lambda\frac{1}{2} \sum_{j=1}^T w_j^2$$ ，其中， $T$ 代表叶子节点数量， $w_j$是叶子节点的预测值， $$\lambda\frac{1}{2} \sum_{j=1}^T w_j^2$$ 为 L2 正则化项。
...
随后，从第 $T$ 步开始回溯，即 $i_{t-1}^\* = \psi_t(i_{t}^\*)$ ，得到完整的隐藏序列 $I=(i_{1}^\*, i_{2}^\*, ..., i_{T}^\*)$
...
随后，根据遗忘门得到的遗忘比例 $f_t$ 和这一层要输入的单元状态 $\tilde{C}_t$，

得到这一层的最终单元状态 $C_t = f_t\*C_{t-1} + i_t\*\tilde{C}_t$。
...
1. Seq2Seq 中的 Attention 机制
    ...
    - 假设当前翻译到的 decoder state 为 $\overrightarrow{s_{t-1}}$，则可以计算该状态与输入的每一个单元 $h_j$ 状态的关联性 $e_{tj} = a(s_{t-1},h_j)$，写成向量形式则为 $\overrightarrow{e_t} = a(\overrightarrow{s_{t-1}}, \overrightarrow{h})$，其中， $a$ 是相关性的计算，常见的计算方式有：
      ...
      - 多层感知机 $a(s_{t-1},h_j)=V \cdot \tanh(W_1 \cdot \overrightarrow{s_{t-1}} + W_2 \cdot \overrightarrow{h})$，其中， 
 $V$、 $W_1$、 $W_2$ 都是可训练矩阵；
...
> Self-Attention 的计算过程如下：
  1. 输入序列 $\overrightarrow{x}$；
  2. 将 $\overrightarrow{x}$ 分别与对应 Q, K, V 的三个可训练矩阵 $W_q$, $W_k$, $W_v$ 点乘，得到 $Q=\overrightarrow{x}\cdot W_q$, $K=\overrightarrow{x}\cdot W_k$, $V=\overrightarrow{x}\cdot W_v$；
  3. 计算 $Attention(Q,K,V)=softmax(\frac{Q\cdot K^T}{\sqrt{d_K}})\cdot V$，其中，$d_K$ 为 $K$ 的维度。
